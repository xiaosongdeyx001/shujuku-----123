...
<div class="container-fluid">
	...
	$(document).ready(function(){
        ...
        function getData(){
            var _options ={
                url:_path+"/invoicing/base/date/area/page/list"
                ,checkAll:false
                //查询条件
                ,data:{'userName':$("[name=shortName]").val()
                	  ,'phone':$("[name=name]").val()}
                ,cloumns:[
					 {name:'地区名称',value:'name'}
                    ,{name:'简称',value:'shortName'}
                    ,{name:'拼音',value:'spelling'}
                    ,{name:'邮编',value:'zip'}
  ...
  //点击：删除
    function delObj(id) {
    	callmodalFun('您确认删除该记录吗？',function(){
    		$.ajax({
    			type : "post",
    			url :_path+"/invoicing/base/date/area/del",
    			data : {
    				'id':id
    			},
    			beforeSend: function () {
    			...
    			success : function(data) {
    				...
    				if (data.code== 1) {
    					alert("地区删除成功！")
    					var url = _path+"/invoicing/base/date/area/list";
    					goBackPage(url);
    				} else if (data == 0) {
    ...
    function toUserRole(userId){
  	  var url=_path+"/invoicing/base/date/area/role?userId="+userId;
    ...
    $("#addBtn").click(function(){
    	var url=_path+"/invoicing/base/date/area/add";
		$.get(url,function(data){
			$("#mian_div").html(data);
		});    
    });
    ...
    function toUpdatePage(userId){
    	 var url=_path+"/invoicing/base/date/area/update?userId="+userId;
		 //调用跳转方法
		 goBackPage(url);
    }
