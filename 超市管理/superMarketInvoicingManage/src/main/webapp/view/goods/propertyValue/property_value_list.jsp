...
<div class="container-fluid">
	<ol class="breadcrumb">
		<span>当前位置：</span>
		<li><a href="/index">商品管理管理</a></li>
		<li><a href="####">商品属性值管理</a></li>
	</ol>
	...
	<!-- 列表：查询分页列表 end -->
	<!-- add by WHao start 引入：商品属性值列表js -->
	...
	$(document).ready(function(){
        ...
        function getData(){
            var _options ={
                url:_path+"/invoicing/goods/property/value/page/list"
                ,checkAll:false
                //查询条件
                ...
                ,cloumns:[
                    ...
                    ,{name:'属性值名称',value:'propertyValue'}
  ...
  //点击：删除
    function delObj(id) {
    	callmodalFun('您确认删除该记录吗？',function(){
    		$.ajax({
    			type : "post",
    			url :_path+"/invoicing/goods/property/value/del",
    			data : {
    				'id':id
    			},
    			beforeSend: function () {
    			...
    			success : function(data) {
    				...
    				if (data.code== 1) {
    					alert("商品属性值删除成功！")
    					var url = _path+"/invoicing/goods/property/value/page/list";
    					goBackPage(url);
    				} else if (data == 0) {
    					timedTaskFun(1000,'商品属性值删除失败','','err');
    				} else if(data == -2) {
    					timedTaskFun(1000,'该商品属性值，已关联其他业务，故无法删除！','','err');
    				}
    ...
    function toUserRole(userId){
  	  var url=_path+"/invoicing/goods/property/value/role?userId="+userId;
    ...
    $("#addBtn").click(function(){
    	var url=_path+"/invoicing/goods/property/value/add";
		$.get(url,function(data){
			$("#mian_div").html(data);
		});    
    });
    ...
    function toUpdatePage(userId){
    	 var url=_path+"/invoicing/goods/property/value/update?userId="+userId;
		 //调用跳转方法
		 goBackPage(url);
    }
